...
export type EditorComboMeterConfig = Pick<
  ComboPluginConfig,
  "enableComboCounter" | "enableComboTimer" | "comboCounterSize"
>;
...
export class EditorComboMeter implements Plugin<EditorComboMeterConfig> {
  ...
  public onDidChangeTextDocument = (
    data: PowermodeChangeTextDocumentEventData,
    event: vscode.TextDocumentChangeEvent
  ) => {
    this.combo = data.currentCombo;
    this.timerDurationInMilliseconds = data.comboTimeout * 1000;
    this.timerExpirationTimestampInMilliseconds =
      new Date().getTime() + this.timerDurationInMilliseconds;
  ...
  private updateDecorations = (editor: vscode.TextEditor) => {
    ...
    const firstVisibleRange = editor.visibleRanges
      .sort()
      .find((range) => !range.isEmpty);
    ...
    if (
      this.combo !== this.renderedComboCount ||
      !range.isEqual(this.renderedRange)
    ) {
  ...
  private getSharedStyles = (
    ...
    const powerModeCombo = this.isPowermodeActive
      ? comboCount - this.initialPowermodeCombo
      : 0;
    ...
    const textSize = this.isPowermodeActive
      ? ((styleCount * baseTextSize) / 100) * Math.pow(0.5, frameCount * 0.2) +
        baseTextSize
      : baseTextSize;
    // Only change color in power mode
    const color = `hsl(${
      100 - (this.isPowermodeActive ? powerModeCombo : 0) * 1.2
    }, 100%, 45%)`;
  ...
  private createComboTimerDecoration(
    ...
    const updateComboTimerDecoration = () => {
      const timeLeft =
        this.timerExpirationTimestampInMilliseconds - new Date().getTime();
      ...
      const createComboTimerBeforeDecoration = (
        lightTheme?: boolean
        ...
        return {
          before: {
            ...
            color: "white",
            height: "8px",
            textDecoration: `none; ${EditorComboMeter.DEFAULT_CSS} ${baseCss} ${
    ...
    this.comboTimerDecorationTimer = setInterval(
      updateComboTimerDecoration,
